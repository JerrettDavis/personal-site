...
import {buildSearchIndex, PageResult, PostResult} from "../lib/search";
                            ...
                            <ul className={styles.resultsList}>
                                {pageResults.map((page) => (
                                    <li className={styles.resultCard} key={page.href}>
                                        <Link href={page.href} className={`${styles.resultLink} glowable`}>
                                            {page.label}
                                        </Link>
                            ...
                            <ul className={styles.resultsList}>
                                {postResults.map((post) => (
                                    <li className={styles.resultCard} key={post.id}>
                                        <Link href={`/blog/posts/${post.id}`} className={`${styles.resultLink} glowable`}>
                                            {post.title}
                                        </Link>
...
export const getStaticProps: GetStaticProps<SearchProps> = async () => {
    const {pages, posts} = await buildSearchIndex();
    return {
        props: {
            pages,
            posts,
