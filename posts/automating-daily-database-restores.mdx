...
make it difficult for various teams to access the data they need for development, testing, reporting, data analysis, etc.
...
Add the following lines to your `Dockerfile` directly after the `USER roo` line:
...
to the latest production data for development, testing, reporting, data analysis, etc. The container can be run on a schedule using a CRON job to ensure that the data is always up-to-date. This solution can be
